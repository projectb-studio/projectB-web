...
CLAUDE.md Section 7 와이어프레임 기준으로 Home 페이지를 구현한다.
Newsletter 섹션은 요구사항에 없으므로 제외.
...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <BrandStoreCta />
      <ProductGridSection title="BEST SELLERS" products={bestSellers} viewAllHref="/shop" />
    </>
  );
}
```
...
- **Props:** `title`, `products`, `viewAllHref`
...
export const HERO_CONTENT = {
  heading: "PROJECT B",
  subtext: "Handcrafted lifestyle goods",
